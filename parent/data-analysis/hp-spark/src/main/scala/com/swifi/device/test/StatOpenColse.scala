...
object StatOpenColse {
  ...
  //路线终始站地理坐标
  case class macRoadInfo(apMac: String,loc1Lat: Double,loc1Lng: Double,loc2Lat: Double,loc2Lng: Double)
  def main(args: Array[String]): Unit = {
    ...
    val sparkConf = new SparkConf().setMaster("local").setAppName("StatOpenColse AppD:/TEST/opencloselication test")
    ...
    val dsdFile = sc.textFile("D:/TEST/data/dsd")
    val lbsFile = sc.textFile("D:/TEST/data/lbs")

    //线路信息
    val roadFile = sc.textFile("D:/TEST/data/mac_road.txt")
    ...
    val temp_2 = sqlContext.sql("select leftCloseTable.*,lbs.lat as cLat,lbs.lng as cLng from leftCloseTable left join lbs " +
      "on leftCloseTable.dsdApMac = lbs.lbsApMac and (leftCloseTable.closeTime - cInter = lbs.time or leftCloseTable.closeTime + cInter = lbs.time)").filter($"cLat".isNotNull).drop("closeStat")
    ...
    result.write.mode(SaveMode.Overwrite).json("D:/TEST/1/")
    //temp_4.write.mode(SaveMode.Overwrite).json("D:/TEST/2/")
  }
